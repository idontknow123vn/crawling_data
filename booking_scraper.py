    ...
    with (sync_playwright() as p):
        ...
        try:
            for i in range(0, 1):
                ...
                time.sleep(5) if i == 0 else time.sleep(2)
                page.locator('//input[@id=":rh:"]').fill(provinces[i])
                time.sleep(1.5)
                page.wait_for_load_state('load')
                list_recommendations = page.locator('//div[@role="button" and @class="d7430561e2"]').all()
                for r in range(0, len(list_recommendations)):
                    if page.locator('//div[@class="a3332d346a d2f04c9037"]').nth(r).inner_text() == provinces[i]:
                        print("true")
                        page.locator('//li[@role="option" and @class="be14df8bfb"]').nth(r).click()
                        break
                    else:
                        print("false")
                time.sleep(0.75)
                ...
                for _ in range(3):
                    page.evaluate(f"window.scrollBy(0, {page_height / 4})")
                    page.wait_for_load_state('load')
                ...
                print(f'There are: {len(hotels)} hotels in {provinces[i]}.')
                ...
                for index, hotel in enumerate(hotels):
                    ...
                    hotel_dict['province'] = provinces[i]
                    ...
                    services = new_page.locator('//span[@class="a5a5a75131"]').all()
                    list_services = ""
                    for service in services:
                        list_services += service.inner_text() + ", "
                    hotel_dict['services'] = list_services
